...
const ClerkUserSync = dynamic(() => import('@/components/ClerkUserSync'), {
  ssr: false
})
...
export function GlobalContextProvider(props) {
  ...
  const [isDarkMode, updateDarkMode] = useState(
    defaultDarkMode === 'dark' || defaultDarkMode === 'true'
  ) // 默认深色模式
  const [onLoading, setOnLoading] = useState(false) // 抓取文章数据
  const router = useRouter()

  // 登录验证相关 — do not call useUser() during SSG.
  // Clerk auth context can be missing at prerender and throws:
  ...
  const onClerkUserChange = useCallback(next => {
    setClerkUser(prev => {
      if (
  ...
  // The initial <html lang> is a build-time fallback. Keep the live document
  // language aligned with the CONFIG-TABLE after hydration.
  useEffect(() => {
  ...
  return (
    <GlobalContext.Provider
      value={contextValue}>
      {enableClerk ? <ClerkUserSync onChange={onClerkUserChange} /> : null}
      {children}
    </GlobalContext.Provider>
  )
}
