...
function isUsableCacheValue(data) {
  ...
  // Do not treat EmptyData error fallback as usable cache value
  if (typeof data === "object" && Array.isArray(data?.allPages) && data.allPages.length === 1 && data.allPages[0]?.id === "oops") {
    return false
  }
  return true
}
  ...
  const promise = getDataFunction(...getDataArgs)
    .then(async data => {
      if (isUsableCacheValue(data)) {
        ...
        cacheLog('SKIP-SET', key, 'unusable/empty data, skip cache overwrite')
      }
...
export async function setDataToCache(key, data, customCacheTime) {
  ...
  // Extract dynamic TTL from customCacheTime or data.NOTION_CONFIG.NEXT_REVALIDATE_SECOND
  let effectiveTtl = customCacheTime
  if (!Number.isFinite(effectiveTtl) || effectiveTtl <= 0) {
    const fromConfig = Number(data?.NOTION_CONFIG?.CONFIG_REVALIDATE_SECOND ?? data?.NOTION_CONFIG?.NEXT_REVALIDATE_SECOND)
    if (Number.isFinite(fromConfig) && fromConfig > 0) {
      effectiveTtl = fromConfig
    }
  }
  ...
  for (const { name, api } of chain) {
    try {
      await api.setCache(key, data, effectiveTtl)
...
/**
 * Clear caches owned by this process. Redis intentionally has no global flush:
 * it may be shared with other applications and cannot be safely wiped without
 * a namespaced invalidation scheme. On-demand revalidation still refreshes
 * Next's ISR route cache separately.
 */
export async function cleanCacheData() {
  inflightMap.clear()
  const cleared = []
  const seen = new Set()

  for (const { name, api } of getCacheChain()) {
