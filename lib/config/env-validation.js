...
const ENV_VALIDATION_RULES = {
  ...
  requiredNotion: {
    NOTION_PAGE_ID: {
      validator: (value) => {
        if (!value) return 'NOTION_PAGE_ID is required for Notion CMS'
  ...
  requiredFeishu: {
    FEISHU_APP_ID: {
      validator: (value) => {
        if (!value || !value.trim()) return 'FEISHU_APP_ID is required for Feishu CMS'
        return true
      }
    },
    FEISHU_APP_SECRET: {
      validator: (value) => {
        if (!value || !value.trim()) return 'FEISHU_APP_SECRET is required for Feishu CMS'
        return true
      }
    }
  },
...
export function validateEnvironmentVariables() {
  ...
  const isFeishu = (process.env.CMS_PROVIDER || '').toLowerCase() === 'feishu' || Boolean(process.env.FEISHU_APP_ID)
  const requiredRules = isFeishu ? ENV_VALIDATION_RULES.requiredFeishu : ENV_VALIDATION_RULES.requiredNotion

  // 验证必需的环境变量
  for (const [key, rules] of Object.entries(requiredRules)) {
