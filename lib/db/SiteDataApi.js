...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  const findPost = () => {
    ...
    const isContent = p => p.type === 'Post' || p.type === 'Page' || p.type === 'Notice'
    const matchSlug = (p, s) => {
      if (!p || !s) return false
      if (p.slug === s || p.href === `/${s}` || p.href === s) return true
      if (p.ext?.nodeToken === s || p.ext?.documentId === s) return true
      // post.slug may be "article/TOKEN" while route segment is TOKEN
      if (typeof p.slug === 'string' && (p.slug.endsWith('/' + s) || p.slug.split('/').pop() === s)) return true
      return false
    }
    const bySlug = (s) =>
      pages.find(p => isContent(p) && matchSlug(p, s) && p.ext?.documentId) ||
      pages.find(p => isContent(p) && matchSlug(p, s)) ||
      pages.find(p => matchSlug(p, s))
    return (
      bySlug(fullSlug) ||
      bySlug(lastSegment) ||
      pages.find(p => p.href === `/${fullSlug}` || p.href === `/${lastSegment}` || p.href === `/article/${lastSegment}`) ||
      pages.find(p => p?.id === fullSlug || p?.id === lastSegment) ||
      null
    )
  }
