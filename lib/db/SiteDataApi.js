...
import { fetchSiteFromFeishu, enrichFeishuPost } from '@/lib/site/adapters/feishu/feishu.adapter'
...
export { fetchNotionPageBlocks as getPostBlocksNotion } from './notion/getPostBlocks'

/** getPostBlocks: no-op for Feishu posts (preview blocks optional). */
export async function getPostBlocks(id, from, slice) {
  const isFeishu = (process.env.CMS_PROVIDER || BLOG.CMS_PROVIDER || 'feishu').toLowerCase() === 'feishu'
  if (isFeishu) {
    // Skip Notion recordMap previews; list cards use title/summary only.
    return { block: {} }
  }
  const { fetchNotionPageBlocks } = await import('./notion/getPostBlocks')
  return fetchNotionPageBlocks(id, from, slice)
}
...
export async function fetchGlobalAllData({
  ...
  const cachedData = await getOrSetDataWithCache(cacheKey, async () => {
    // Feishu CMS path
    const cms = process.env.CMS_PROVIDER || BLOG.CMS_PROVIDER || ''
    if (String(cms).toLowerCase() === 'feishu' || process.env.FEISHU_APP_ID) {
      if (String(cms).toLowerCase() === 'feishu') {
        try {
          const data = await fetchSiteFromFeishu()
          return handleDataBeforeReturn(deepClone(data))
        } catch (error) {
          console.error('[feishu] fetchSiteFromFeishu failed', error)
          const staleData = await getDataFromCache(cacheKey, true)
          if (staleData && Array.isArray(staleData.allPages) && staleData.allPages.length > 1) {
            console.warn('[feishu] Serving stale global data due to fetch error')
            return staleData
          }
          return handleDataBeforeReturn(deepClone(EmptyData(pageId)))
        }
      }
    }
...
function getGlobalDataCacheKey({ pageId, locale }) {
  ...
  // Feishu /en is a UI locale switch; content comes from one table.
...
async function getNotice(post) {
  if (!post) return null
  // Feishu CMS: notice already enriched with feishuContent
  if (post?.ext?.source === 'feishu' || post?.feishuContent || process.env.CMS_PROVIDER === 'feishu') {
    return post
  }
...
/**
 * Apply the same optional preview/summary enrichment to every post list.
 * Callers should filter and paginate first so the work matches the visible list.
 */
export async function enrichListPosts(posts, NOTION_CONFIG) {
  if (!Array.isArray(posts)) return posts

  const postListPreview = siteConfig('POST_LIST_PREVIEW', false, NOTION_CONFIG)
  if (!postListPreview) return posts

  const previewLines = siteConfig('POST_PREVIEW_LINES', 8, NOTION_CONFIG)
  const previewMaxCount = siteConfig('POST_PREVIEW_MAX_COUNT', 4, NOTION_CONFIG)
  const previewLimit = pLimit(
    siteConfig('POST_PREVIEW_CONCURRENCY', 5, NOTION_CONFIG)
  )
  const previewTargets = posts
    .map((post, index) => ({ post, index }))
    .filter(({ post }) => !post.password || post.password === '')
    .slice(0, previewMaxCount)

  const isFeishu =
    (process.env.CMS_PROVIDER || BLOG.CMS_PROVIDER || 'feishu').toLowerCase() ===
    'feishu'
  ...
  if (isFeishu) {
    ...
    await Promise.all(
      previewTargets.map(({ post, index }) =>
        previewLimit(async () => {
          posts[index] = await enrichFeishuPost(post)
        })
      )
    )
  } else {
    await Promise.all(
      previewTargets.map(({ post }) =>
        previewLimit(async () => {
          const rawBlockMap = await getPostBlocks(post.id, 'slug', previewLines)
...
function cleanNoticeForClient(notice) {
  // Feishu notice body is feishuContent, not Notion blockMap
  if (notice?.feishuContent || notice?.ext?.source === 'feishu') {
    return notice
  }
...
const EmptyData = pageId => ({
  ...
  allPages: [
    {
      id: 'oops',
      title: `无法获取飞书数据，请检查 CMS_PROVIDER / 飞书应用与内容表配置： \n 当前 ${pageId}`,
      summary:
        '请检查 FEISHU_APP_ID、内容表授权，或查阅仓库 docs/feishu/',
      status: 'Published',
      type: 'Post',
      slug: 'oops',
...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  const findPost = () => {
    ...
    const pages = props.allPages.filter(
      p => p && p.type !== 'Menu' && p.type !== 'SubMenu' && !String(p.type || '').includes('Menu')
    )
    const isContent = p => (p.type === 'Post' || p.type === 'Page' || p.type === 'Notice') && p.status === 'Published'

    const exactMatch = s => {
      if (!s) return null
      return (
        pages.find(p => isContent(p) && (p.slug === s || p.href === `/${s}` || p.href === s || p.id === s || p.ext?.nodeToken === s || p.ext?.documentId === s)) ||
        pages.find(p => p.slug === s || p.href === `/${s}` || p.href === s || p.id === s || p.ext?.nodeToken === s || p.ext?.documentId === s)
      )
    }

    // 1. 优先 fullSlug 精确匹配
    let match = exactMatch(fullSlug)
    if (match) return match

    // 2. 尝试 lastSegment 精确匹配（如单段 token / docToken / slug）
    if (lastSegment && lastSegment !== fullSlug) {
      match = exactMatch(lastSegment)
      if (match) return match
    }

    // 3. 尝试 /article/TOKEN 格式
    if (lastSegment) {
      match = pages.find(p => isContent(p) && (p.href === `/article/${lastSegment}` || p.slug === `article/${lastSegment}`))
      if (match) return match
    }
    ...
    if (lastSegment) {
      match = pages.find(
        p => isContent(p) && typeof p.slug === 'string' && (p.slug.endsWith('/' + lastSegment) || p.slug.split('/').pop() === lastSegment)
      )
      if (match) return match
    }
  ...
  const ensureBlockMap = async (post) => {
    if (!post) return post
    const isFeishu =
      post?.ext?.source === 'feishu' ||
      (process.env.CMS_PROVIDER || BLOG.CMS_PROVIDER || 'feishu').toLowerCase() === 'feishu'
    // Feishu: load docx blocks into feishuContent instead of notion recordMap
    if (isFeishu) {
      const step4Start = Date.now()
      try {
        post = await enrichFeishuPost(post)
      } catch (e) {
        console.warn(`[${taskId}] [resolvePostProps] enrichFeishuPost failed:`, post.id, e)
      }
      const step4End = Date.now()
      console.log(`[${taskId}] ⏱ enrichFeishuPost 耗时: ${step4End - step4Start}ms @ ${new Date().toISOString()}`)
      return post
    }
...
function cleanTagOptions(tagOptions) {
  if (tagOptions && Array.isArray(tagOptions)) {
    return tagOptions
      .filter(tagOption => !tagOption.source || tagOption.source === 'Published')
...
function getSiteInfo({ collection, block, rawMetadata, NOTION_CONFIG }) {
  const defaultTitle = NOTION_CONFIG?.TITLE || 'FeiShow'
  const defaultDescription =
    NOTION_CONFIG?.DESCRIPTION || '这是一个由 FeiShow 生成的站点'
