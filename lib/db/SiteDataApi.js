...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  const findPost = () => {
    ...
    return (
      ...
      // 1b. 最后一段 slug（如 article/xxx → xxx）
      props.allPages.find(p => p && !p.type?.includes('Menu') && p.slug === lastSegment) ||
      // 1c. href 匹配
      props.allPages.find(p => p && (p.href === `/${fullSlug}` || p.href === `/${lastSegment}` || p.href === `/article/${lastSegment}`)) ||
      // 2. UUID 匹配
