...
function writeActiveTheme(theme, source) {
  ...
  for (const item of candidates) {
    const raw =
      typeof item === 'object' && item !== null
        ? String(item.token || item.link || item.url || item.text || '')
        : String(item || '')
    if (!raw) continue
    const match = raw.match(/\/(?:wiki|docx|docs|doc|base|bitable)\/([A-Za-z0-9_-]+)/)
...
async function findConfigTable(api, content) {
  ...
  for (let page = 0; page < 5; page += 1) {
    ...
    for (const record of data.items || []) {
      ...
      const rawDoc = docTokenFromField(record.fields?.['文档'] ?? record.fields?.document)
      if (!rawDoc) continue
      ...
      if (embedded) {
        ...
        const linkMatch = rawDoc.match(/\/(?:base|bitable)\/([A-Za-z0-9]+).*?[?&]table=(tbl[A-Za-z0-9]+)/)
        if (linkMatch) {
          appToken = linkMatch[1]
          tableId = linkMatch[2]
        }
      }
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    try {
      ...
      try {
        ...
        writeActiveTheme(theme, 'config-table')
      } catch (error) {
